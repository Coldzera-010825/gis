...
*(Code in §4 below — it is a single call once the SHAP values exist.)*
```
...
This is the *entire* code behind the hero figure — the work all lives in the
`Explanation` object from §2; the plot itself is one call plus an export:

```python
shap.plots.beeswarm(sv, max_display=8, show=False)   # draws onto the current axes

fig = plt.gcf()                                      # grab it for styling/export
fig.set_size_inches(8.6, 5.4)
fig.savefig("shap-main-beeswarm.jpg", dpi=150, bbox_inches="tight")
```
